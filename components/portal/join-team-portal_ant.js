...
export const JoinTeamPortal = props => {
  ...
  const { user, users, status, error, onRegister} = useContext(AuthenticationContext);
  ...
  const onPay = () => {           
     ...
     // if (ultNumberCard==="")   { setErrorCard('Entry number card'); return ''; }
      ...
      if (isRegister && statusUser!=='success') { 
        ...
        idStr = id.toString();              
        let ok = onRegister(
                  {"id": idStr, "name": name, "email": email, 
                  "password": password, "repeatedPassword": password, "gdpr": saveGDPR});  
        if (ok==="Ok") {
          setStatusUser('success');
          setShowUser(false);
          success = true;
      ...
      //statusUser==="success"
      if (success && reservaId==='') {
        //El usuario ya existia o lo acabamos de registrar, es una nueva reserva  nuevo paymentIntent                 
        let resId = setReservation({"endDate": endDate, "room_id": roomId, "startDate": startDate, 
            "status": "PENDING", "user_id": idUser });
    ...
    <View style={style.containerArea}>     
      ...
      {!isLoading && !nerror && (  
        <ScrollView style={{marginLeft: 10, height:'100%'}}        
          ...
          {error && statusUser==="error" && (<View style={style.error}>          
              <Text>{error}</Text>
            </View>
          )} 
