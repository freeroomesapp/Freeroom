...
import { Text, View, ScrollView, Image, Platform, Dimensions, TouchableOpacity, 
         FlatList, RefreshControl, Keyboard } from 'react-native';
import { Appbar, TextInput, Portal, IconButton, ActivityIndicator, Badge } from 'react-native-paper';
...
import { hour, dns, hotels, districts } from '../hooks/utils/data';
import { CardDrawerGesture } from '../components/card-Drawer';  
...
export const ScreenA = () => {
  ...
  const getIconExpand = () =>{
    ...
    if (expand==='vertical') return 'arrow-expand-down'; //'arrow-down'; //'arrow-expand-vertical'
    else if (expand==='down') return 'arrow-expand-up'; //'arrow-up'; //
  }
        ...
        fetch(dns + '/free-room/v1/hotels')
          ...
          .then((json) => { 
            ...
            if (json.data.length>0) {
              console.log('Cantidad inicial de hoteles: '+json.data.length);
              ...
              console.log('Cantidad de hoteles disponibles: '+newItems.length);
              let newNearby = newItems.sort((a, b) => (getDistanceCurrent(a)>getDistanceCurrent(b))?1 : -1).filter((data,idx) => idx < 8)
              setUsers(newNearby);
            } else {
              setUsers(hotels.sort((a, b) => (getDistanceCurrent(a)>getDistanceCurrent(b))?1 : -1).filter((data,idx) => idx < 7));
              console.log('-Cantidad de hoteles de pruebas: '+hotels.length);
          ...
          setUsers(hotels.sort((a, b) => (getDistanceCurrent(a)>getDistanceCurrent(b))?1 : -1).filter((data,idx) => idx < 8));
          console.log('Cantidad de hoteles de pruebas: '+hotels.length);
          setMsg2('Se  usaran datos de prueba.');
          })
  ...
  const { favico, containerFlat, containerMarket, triangle, card, cardImage, 
          cardtitle, cardDescription, containerImageIcon, iconButton, iconTopLeft, 
          badge, currencyText, currencyTextOffer, currencyTextNot, distanceText, 
          currencyRowText, currencyRowTextOffer } = Cardstyles();

  useFocusEffect(
    useCallback(() => {
      return () => {
        if (joinPortalDialogVisible && screens.send==="ScreenA") {
          handleHideAll();
          removeScreens();
        }
      };
  ...
  useEffect(() => {
    if (joinPortalDialogVisible && screens.send==="ScreenA") setScreen(true);
  ...
  const onButtonPress = useCallback(() => {
    if (screens.send==="") {
      ...
      addScreens({"send": "ScreenA", "receive": "JoinTeamPortal"});
    }
  }, [handleShow]);
  ...
  //
  const [isModal, setIsModal] = useState(true);//Platform.OS!=="android"?true:false
  ...
  const getDistanceCurrentText = (item) => {
  ...
  const renderItem = ({ item, index }) => {         
    let i = 0; var rooms = null;     
    if (item.rooms && item.rooms.length>0) { 
      rooms = item.rooms.filter((end) => end.availability===1); 
      i = rooms.length;
      //rooms = rooms.filter((end,idx) => (end.availability===1));
      //rooms.map((end, index) => {if (end.availability===1) i = i + 1; })
    } 
    if (i===0) return '';    
    var hours = new Date().getHours(); //Current Hours 
    if (hours >= hour) {
      rooms.sort((a, b) => (a.offerPrice > b.offerPrice) ? 1 : -1);
    } else {
      rooms.sort((a, b) => (a.price > b.price) ? 1 : -1);
    }
    var price  = getFormatCurrency(rooms[0].price); 
    var priceI = getFormatCurrency(rooms[0].price);     
    let showoffer = false;
    if (rooms[0].onSale===1 && hours >= hour) {
      showoffer = true;
      priceI = getFormatCurrency(rooms[0].offerPrice);
    }
    //
    return (
    <TouchableOpacity key={index} style={card}
        ...
        <View style={containerImageIcon}>
          ...
          {showoffer && (
            <View>              
              <Text style={currencyTextOffer}> {priceI} </Text>
  ...
  <>    
    ...
    {!runScreen && isLoading && Platform.OS ==="web" && (<MapSkeleton text1={error} text2={msg2} />)}    
    {!runScreen && isLoading && Platform.OS !=="web" && (
      <View style={{top:100}}>
        <ActivityIndicator  animating={true}  size="large"/>      
      </View>
    )}    
    {!runScreen && !isLoading && (Platform.OS === "android" || Platform.OS === "ios") && (              
      <View style={styles.container}>            
              ...
              <CalloutMob tooltip>
                  <View>
                    ...
                    <View style={styles.callout}>
                      <Text style={{fontSize: 10 }} >Arrastre para encontrar hoteles cerca de mi ubicación</Text>  
        ...
        {users.map((data, index) => {  
          ...
          var rooms = data.rooms.filter((end) => end.availability===1); 
          var hours = new Date().getHours(); //Current Hours 
          if (hours >= hour) {
            rooms.sort((a, b) => (a.offerPrice > b.offerPrice) ? 1 : -1);
          } else {
          ...
          >    
            {showoffer && (<Text style={currencyRowTextOffer}> {priceI} </Text>)} 
            {!showoffer && (<Text style={currencyRowText}> {price} </Text>)}            
    ...
    filteredResult.length==1?styles.setHeight1:filteredResult.length==2?styles.setHeight2:''
        ...
        <View style={styles.buttonContainer}>                        
          <View style={[expand==='down'?styles.setHeight:'', 
                        expand==='vertical'?containerFlat:'' , { flex: 1 }]}>
              <View style={{alignItems: "center"}}>               
...
const styles = StyleSheet.create({
  ...
  setHeight: {
    height: CARD_HEIGHT*0.4, //0.35
    marginVertical: -34, //Para que el icon quede arriba de la view.
  },
  setHeight1: {
